...
void multiply_matrix2(double* matrixA, double* matrixB, double* matrixC)
{
#pragma omp parallel for schedule(static, 10) num_threads(2)
    for (int i = 1; i < SIZE; i++)
        for (int j = 0; j < SIZE; j++)
//#pragma omp reduction(+: matrixC[i * SIZE + j])
            for (int k = 0; k < SIZE; k++)
                matrixC[i * SIZE + j] += matrixA[k + i * SIZE] * matrixB[j + k * SIZE];
}


void multiply_matrix4(double* matrixA, double* matrixB, double* matrixC)
{
...
void multiply_matrix6(double* matrixA, double* matrixB, double* matrixC)
{
...
int main()
{
    ...
    {
        auto start_time = std::chrono::steady_clock::now();
        multiply_matrix2(matrixA, matrixB, matrixC);
    ...
    {
        auto start_time = std::chrono::steady_clock::now();
        multiply_matrix4(matrixA, matrixB, matrixC);
    ...
    {
        auto start_time = std::chrono::steady_clock::now();
        multiply_matrix6(matrixA, matrixB, matrixC);
